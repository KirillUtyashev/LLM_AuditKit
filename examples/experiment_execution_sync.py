"""Run one small hiring experiment through the synchronous execution API."""
...
def main() -> int:
    if len(sys.argv) != 2:
        print(
            "Usage: python examples/experiment_execution_sync.py OUTPUT.csv",
            file=sys.stderr,
        )
        return 2
    ...
    dataset = _dataset()
    config = _config("example-sync-v1")
